...
from swagger_spec_compatibility.rules.removed_nullable_property_from_request import RemovedNullablePropertyFromRequest
from swagger_spec_compatibility.spec_utils import load_spec_from_spec_dict


def test_validate_succeed(minimal_spec):
    assert list(RemovedNullablePropertyFromRequest.validate(
...
def test_validate_return_an_error(
    ...
    expected_results = [
        RemovedNullablePropertyFromRequest.validation_message(
...
def test_validate_does_not_error_if_changes_in_response_schema(minimal_spec_dict):
            ...
            '/endpoint': {
                'get': {
                    'responses': {
                        '200': {
                            'description': '',
                            'schema': {'type': 'object', 'x-nullable': True},
                        },
                    },
                },
            },
    ...
    new_spec_dict = deepcopy(old_spec_dict)
    del new_spec_dict['paths']['/endpoint']['get']['responses']['200']['schema']['x-nullable']
...
def test_validate_does_not_error_if_changes_in_top_level_parameters(minimal_spec_dict):
    ...
    new_spec_dict = deepcopy(old_spec_dict)
    del new_spec_dict['parameters']['param']['schema']['x-nullable']
...
def test_validate_fails_if_parameters_are_defined_in_different_locations_with_different_required_properties(
    ...
    del new_spec_dict['paths']['/endpoint']['get']['parameters'][0]['schema']['x-nullable']
    new_spec = load_spec_from_spec_dict(new_spec_dict)
    ...
    assert list(RemovedNullablePropertyFromRequest.validate(
        ...
        RemovedNullablePropertyFromRequest.validation_message(reference='#/paths//endpoint/get/parameters/0/schema/x-nullable'),
    ]
